...
def upgrade():
    ...
    with op.batch_alter_table("user", schema=None) as user_op:
        user_op.add_column(
            sa.Column("fs_webauthn_user_handle", sa.String(64), nullable=True)
        )
        user_op.add_column(sa.Column("mf_recovery_codes", sa.TEXT, nullable=True))
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    with op.batch_alter_table("user", schema=None) as user_op:
        user_op.drop_column("fs_webauthn_user_handle")
        user_op.drop_column("mf_recovery_codes")
