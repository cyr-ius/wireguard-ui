...
from ..models import OidcSettings, Role, User
...
def build_token_request(
    discovery: dict[str, Any], settings: OidcSettings, code: str
...
def to_oidc_response(settings: OidcSettings) -> OidcSettingsResponse:
    """Map OidcSettings to OidcSettingsResponse."""
    return OidcSettingsResponse.model_validate(settings)
...
async def get_or_create_oidc_settings(db: AsyncSession) -> OidcSettings:
    ...
    if settings is None:
        settings = OidcSettings.model_validate(OIDC_DEFAULT)
        db.add(settings)
        await db.commit()
